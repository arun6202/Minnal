...
| 5 | llama.cpp vendored as git submodule or cargo FFI crate? | **`llama-cpp-2` 0.1.146, CPU-only for Spike B. DirectML deferred to Phase 2 M5.** | `llama-cpp-2` exposes no DirectML feature in 0.1.146. CPU-only closes Spike B and unblocks Phase 1. DirectML (Intel Xe path) requires a newer `llama-cpp-2` release or a custom `build.rs` against a vendored llama.cpp submodule — deferred to Phase 2 M5 where the full backend abstraction is implemented. |
